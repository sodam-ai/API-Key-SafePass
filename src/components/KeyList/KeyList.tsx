...
export default function KeyList({ projectId, platformName, isAllView, projects, keys, onKeysChanged, onAddKey }: KeyListProps) {
  ...
  const handleCopy = async (keyId: string) => {
    try {
      ...
      setCopiedId(keyId);
      onKeysChanged(); // refresh last_used_at
    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {selectedProject && !isPlatformView && (
            <span
              className="w-3 h-3 rounded-full"
              style={{ backgroundColor: selectedProject.color || "#868e96" }}
            />
          )}
          {isPlatformView && (
            <span className="w-3 h-3 rounded-sm bg-vault-600/60" />
          )}
          <h1 className="text-lg font-semibold">{viewTitle}</h1>
          <span className="text-sm text-zinc-500">{keys.length}개</span>
        </div>
        <div className="flex items-center gap-2">
          {projectId && keys.length > 0 && (
            <button
              ...
              disabled={exporting}
              className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-sm transition-colors flex items-center gap-1.5 text-zinc-300"
              title=".env 파일 내보내기"
            >
          ...
          <button
            ...
            className="px-3 py-1.5 bg-vault-600 hover:bg-vault-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {keys.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-zinc-500">
            <div className="w-20 h-20 mb-4 rounded-2xl bg-zinc-800/50 border border-zinc-700/50 flex items-center justify-center">
            ...
            <button
              ...
              className="px-6 py-3 bg-vault-600 hover:bg-vault-700 rounded-xl text-sm font-medium transition-colors flex items-center gap-2"
            >
          ...
          <div className="space-y-2">
            {keys.map((k) => {
              ...
              return (
                <div
                  key={k.id}
                  className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 hover:border-zinc-700 transition-colors group"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-medium text-sm truncate">{k.name}</h3>
                        {k.provider && (
                        ...
                        {k.env_var_name && (
                          <span className="px-2 py-0.5 bg-zinc-800/50 rounded text-xs text-zinc-500 font-mono flex-shrink-0">
                            {k.env_var_name}
                          </span>
                        )}
                        {days !== null && (
                          <span className={`px-2 py-0.5 rounded border text-xs font-mono font-bold ${expiryBg(days)} ${expiryColor(days)}`}>
                            {expiryText(days)}
                          </span>
                        )}
                      </div>
                      {k.memo && (
                        <p className="text-xs text-zinc-500 mb-2">{k.memo}</p>
                      )}
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {k.tags.map((tag) => (
                          <span
                            key={tag.id}
                            className="px-2 py-0.5 bg-zinc-800/80 rounded-full text-xs text-zinc-400"
                          >
                            #{tag.name}
                          </span>
                        ))}
                        {k.updated_at && k.updated_at !== k.created_at && (
                          <span className="text-xs text-zinc-600">
                            변경: {relativeDate(k.updated_at)}
                          </span>
                        )}
                        {k.last_used_at && (
                          <span className="text-xs text-zinc-600">
                            사용: {relativeDate(k.last_used_at)}
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 ml-3 flex-shrink-0">
                      ...
                      {days !== null && days < 0 && (
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg text-xs font-medium bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-800/50 transition-colors"
                          title="만료된 키 재발급"
                        >
                          재발급
                        </button>
                      ...
                      {k.service_url && (
                        <button
                          onClick={() => handleOpenServiceUrl(k.service_url!)}
                          className="p-1.5 rounded hover:bg-zinc-700 text-zinc-500 hover:text-vault-400 transition-colors"
                          title="키 관리 페이지 열기"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      >
                        {copiedId === k.id ? "복사됨!" : "복사"}
                      </button>
                      {/* 키값 변경 (빠른) */}
                      <button
                        onClick={() => setQuickUpdateKey(k)}
                        className="p-1.5 rounded hover:bg-vault-600/30 text-zinc-500 hover:text-vault-400 opacity-0 group-hover:opacity-100 transition-all"
                        title="키값 변경"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <button
                        ...
                        className="p-1.5 rounded hover:bg-zinc-700 text-zinc-500 hover:text-zinc-300 opacity-0 group-hover:opacity-100 transition-all"
                        title="전체 수정"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
